...
begin
  ...
  if not v_has_table then return; end if;
  ...
  v_event_name_expr := case when v_has_event_name then 'ue.event_name' when v_has_event_key then 'ue.event_key' else $q$'unknown'::text$q$ end;
  v_feature_area_expr := case when v_has_event_type and v_has_properties then $q$coalesce(ue.event_type, ue.properties->>'feature_area', 'unknown')$q$ when v_has_event_type then $q$coalesce(ue.event_type, 'unknown')$q$ when v_has_properties then $q$coalesce(ue.properties->>'feature_area', 'unknown')$q$ else $q$'unknown'::text$q$ end;
  ...
  v_sql := 'select ' || v_event_name_expr || ' as event_name,' || v_feature_area_expr || ' as feature_area,' || v_platform_expr || ' as platform,' || v_app_version_expr || ' as app_version,' || v_country_expr || ' as country,' || 'count(*)::bigint as event_count,' || v_unique_user_expr || ' as unique_user_count,' || v_first_seen_expr || ' as first_seen_at,' || v_last_seen_expr || ' as last_seen_at ' || 'from public.usage_events ue ' || 'group by 1,2,3,4,5 ' || 'order by event_count desc';
...
begin
  ...
  if not v_has_entitlements then return; end if;
  ...
  v_sql := 'with failed_payers as (' || case when v_has_sub_events and v_has_se_user_id and v_has_se_created_at and v_has_se_event_key then 'select distinct se.user_id from public.subscription_events se where se.created_at > now() - interval ''90 days'' and (se.event_key ilike ''%fail%'' or se.event_key ilike ''%past_due%'')' else 'select null::uuid as user_id where false' end || ') ' || 'select ' || v_plan_expr || ' as plan,' || v_status_expr || ' as billing_status,' || v_provider_expr || ' as subscription_provider,' || 'count(*)::bigint as user_count,' || case when v_has_status then 'count(*) filter (where e.subscription_status in (''active'',''trialing''))::bigint' else '0::bigint' end || ' as active_count,' || case when v_has_status then 'count(*) filter (where e.subscription_status in (''canceled'',''expired''))::bigint' else '0::bigint' end || ' as cancelled_count,' || case when v_has_user_id then 'count(*) filter (where e.user_id in (select user_id from failed_payers))::bigint' else '0::bigint' end || ' as failed_payment_count ' || 'from public.user_entitlements e group by 1,2,3 order by user_count desc';
  return query execute v_sql;
end;
$$;
...
begin
  ...
  if not v_has_usage then return; end if;
  v_has_user_id := public._admin_safe_column_exists('public', 'usage_events', 'user_id');
  v_has_created_at := public._admin_safe_column_exists('public', 'usage_events', 'created_at');
  v_has_properties := public._admin_safe_column_exists('public', 'usage_events', 'properties');
  ...
  v_tokens_expr := case when v_has_tok_in and v_has_tok_out then 'coalesce(ue.estimated_tokens_input,0) + coalesce(ue.estimated_tokens_output,0)' when v_has_tok_in then 'coalesce(ue.estimated_tokens_input,0)' when v_has_tok_out then 'coalesce(ue.estimated_tokens_output,0)' else '0' end;
  ...
  v_storage_join_sql := case when v_has_docs and v_has_doc_owner and v_has_doc_size and v_has_user_id and v_has_created_at then 'storage_by_country as (with user_country as (select distinct on (ue.user_id) ue.user_id, ' || v_country_expr || ' as country_raw from public.usage_events ue where ue.user_id is not null order by ue.user_id, ue.created_at desc) select uc.country_raw, coalesce(sum(md.file_size),0)::bigint as storage_bytes from user_country uc join public.medical_documents md on md.owner_user_id = uc.user_id where md.file_size is not null group by 1)' else 'storage_by_country as (select null::text as country_raw, 0::bigint as storage_bytes where false)' end;
  v_sql := 'with base as (select ' || v_country_expr || ' as country_raw,' || v_user_id_expr || ' as user_id,' || v_created_at_expr || ' as created_at,' || v_tokens_expr || ' as tokens from public.usage_events ue), per_country as (select country_raw, ' || (case when v_has_user_id then 'count(distinct user_id)::bigint' else '0::bigint' end) || ' as user_count, ' || (case when v_has_user_id and v_has_created_at then "count(distinct user_id) filter (where created_at > now() - interval '30 days')::bigint" else '0::bigint' end) || ' as active_user_count, count(*)::bigint as usage_event_count, coalesce(sum(tokens),0)::bigint as ai_tokens_used from base group by 1), ' || v_storage_join_sql || ' select ' || "case when pc.user_count < 10 then 'Other' else pc.country_raw end as country," || 'sum(pc.user_count)::bigint as user_count, sum(pc.active_user_count)::bigint as active_user_count, sum(pc.usage_event_count)::bigint as usage_event_count, coalesce(sum((sbc.storage_bytes / 1048576)::bigint),0)::bigint as storage_used_mb, sum(pc.ai_tokens_used)::bigint as ai_tokens_used from per_country pc left join storage_by_country sbc on sbc.country_raw = pc.country_raw group by 1 order by user_count desc';
  return query execute v_sql;
end;
$$;
...
begin
  ...
  v_sql := 'select ' || case when v_has_usage and v_has_usage_created_at then "(select count(*)::bigint from public.usage_events ue where ue.created_at > now() - interval '24 hours')" else '0::bigint' end || ' as recent_usage_events_24h, ' || case when v_has_usage and v_has_usage_created_at and (v_has_usage_success or v_has_usage_failure_code) then '(select count(*)::bigint from public.usage_events ue where ue.created_at > now() - interval ''24 hours'' and (' || case when v_has_usage_success and v_has_usage_failure_code then '(ue.success is false or ue.failure_code is not null)' when v_has_usage_success then '(ue.success is false)' else '(ue.failure_code is not null)' end || '))' else '0::bigint' end || ' as recent_errors_24h, ' || case when v_has_usage and v_has_usage_created_at and v_has_usage_event_key and (v_has_usage_success or v_has_usage_failure_code) then '(select count(*)::bigint from public.usage_events ue where ue.created_at > now() - interval ''24 hours'' and ue.event_key ilike ''%upload%'' and (' || case when v_has_usage_success and v_has_usage_failure_code then '(ue.success is false or ue.failure_code is not null)' when v_has_usage_success then '(ue.success is false)' else '(ue.failure_code is not null)' end || '))' else '0::bigint' end || ' as failed_upload_events_24h, ' || case when v_has_usage and v_has_usage_created_at and v_has_usage_event_key and (v_has_usage_success or v_has_usage_failure_code) then '(select count(*)::bigint from public.usage_events ue where ue.created_at > now() - interval ''24 hours'' and ue.event_key ilike ''%sync%'' and (' || case when v_has_usage_success and v_has_usage_failure_code then '(ue.success is false or ue.failure_code is not null)' when v_has_usage_success then '(ue.success is false)' else '(ue.failure_code is not null)' end || '))' else '0::bigint' end || ' as failed_sync_events_24h, ' || case when v_has_usage and v_has_usage_created_at then '(select max(ue.created_at) from public.usage_events ue)' else 'null::timestamptz' end || ' as latest_usage_event_at, ' || case when v_has_audit and v_has_audit_created_at then '(select max(al.created_at) from public.admin_audit_log al)' else 'null::timestamptz' end || ' as latest_audit_event_at, ' || case when v_has_support and v_has_support_opened_at then '(select max(ss.opened_at) from public.admin_support_sessions ss)' else 'null::timestamptz' end || ' as latest_support_session_at';
  return query execute v_sql;
end;
$$;
