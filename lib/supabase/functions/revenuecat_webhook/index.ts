...
//
// RevenueCat webhook receiver + entitlement sync.
// - Verifies shared secret (POST only)
// - Idempotent by revenuecat_event_id
// - Stores *redacted* raw payload
// - Inserts lifecycle events into subscription_events
// - Upserts user_entitlements for app access
//
// IMPORTANT PRIVACY:
// - Never log PHI.
...
function redactPayload(payload: AnyJson): AnyJson {
  // RevenueCat payloads can contain a lot of fields. We store a small allow-listed
  // subset for debugging without storing sensitive data.
  ...
  // Some RC events nest entitlement ids differently. Try a couple safe paths.
...
function deriveStatus(eventType: string, expirationAt: string | null): string {
  ...
  if (t.includes("INITIAL_PURCHASE") || t.includes("RENEWAL") || t.includes("UNCANCELLATION") || t.includes("PRODUCT_CHANGE") || t.includes("NON_RENEWING_PURCHASE")) {
    return "active";
  }
  return "unknown";
}
...
serve(async (req) => {
  ...
  // Public, privacy-safe deployment check.
  if (req.method === "GET") {
  ...
  // Verify shared secret (POST only).
  ...
  // Idempotency: if we've already processed this event id, return success.
  try {
    ...
    // If the table is missing or schema cache isn't ready, surface a safe error.
  ...
  // Insert raw event first (unprocessed).
  const insertRow = {
  ...
  try {
    ...
    // Race: another invocation could have inserted. Treat as duplicate.
  ...
  try {
    ...
    try {
      ...
      // subscription_events may not exist yet in some environments.
      processingResult = "warning:subscription_events_insert_failed";
    }

    // Upsert user_entitlements when app user id is mapped to a uuid.
    if (appUserId) {
  ...
  try {
    ...
    // If update fails, still return 200 so RC doesn't retry forever.
  }
