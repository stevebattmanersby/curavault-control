  ...
  Widget build(BuildContext context) {
    ...
    final outerShadow = glow && !reduceMotion
        ? [
            ...tokens.glowShadow,
            BoxShadow(
                color: tokens.secondary.withValues(alpha: 0.10),
                blurRadius: 30,
                spreadRadius: -8,
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: tokens.surfaceElevated
            .withValues(alpha: store.mode == AdminThemeMode.ai ? 0.55 : 1),
        borderRadius: BorderRadius.circular(999),
        border: Border.all(
            color: glow
                ? tokens.borderGlow.withValues(alpha: 0.70)
                : tokens.border,
            width: 1),
  ...
  Widget build(BuildContext context) {
    ...
    return SegmentedButton<AdminThemeMode>(
      ...
      style: ButtonStyle(
        ...
        overlayColor:
            WidgetStatePropertyAll(tokens.primary.withValues(alpha: 0.06)),
        backgroundColor: WidgetStateProperty.resolveWith((states) {
          if (states.contains(WidgetState.selected)) {
            if (selected == AdminThemeMode.ai) {
              return tokens.primary.withValues(alpha: 0.22);
            }
            if (selected == AdminThemeMode.dark) {
      ...
      segments: [
        ButtonSegment(
          ...
          label: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            if (!compact) Text('Light', style: labelStyle)
          ]),
          tooltip: 'Light theme',
        ),
        ButtonSegment(
          ...
          label: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.nightlight_outlined, size: iconSize),
            if (!compact) const SizedBox(width: 8),
            if (!compact) Text('Dark', style: labelStyle)
          ]),
          tooltip: 'Dark theme',
        ),
        ButtonSegment(
          ...
          label: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            if (!compact) const SizedBox(width: 8),
            if (!compact) Text('AI', style: labelStyle)
          ]),
          tooltip: 'AI theme',
        ),
      ],
