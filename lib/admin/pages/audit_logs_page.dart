...
class _AuditLogsPageState extends State<AuditLogsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final canExport =
        auth.role != null && AdminRbac.canExportAuditCsv(auth.role!);

    return AdminPageScaffold(
      title: 'Audit Logs',
      subtitle:
          'Mandatory admin audit trail (metadata only, redacted as needed).',
      actions: [
        AdminDataSourceBadge(
            status: store.dataSource(AdminDataSourceKey.auditLogs)),
          ...
          TextButton.icon(
            onPressed: logs.isEmpty
                ...
                : () async {
                    ...
                    await AdminFileSaver.saveTextFile(
                        filename: 'curavault_audit_logs_$stamp.csv',
                        contents: csv);
                    if (!context.mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Audit log CSV exported${_exportHintSuffix(context)}')),
                    );
                  },
        ...
        IconButton(
          onPressed:
              store.isAuditLogsLoading ? null : () => store.refreshAuditLogs(),
      ...
      child: Column(
        children: [
          AdminOwnerDataSourcePanel(
              store: store,
              dataSourceKey: AdminDataSourceKey.auditLogs,
              title: 'Audit Logs'),
          const SizedBox(height: AppSpacing.md),
          if (store.dataSource(AdminDataSourceKey.auditLogs).kind ==
            ...
            if (store.dataSource(AdminDataSourceKey.auditLogs).kind ==
              ...
              Padding(
                ...
                child: AdminCard(
                  child: Text(
                      store
                              .dataSource(AdminDataSourceKey.auditLogs)
                              .safeErrorMessage ??
                    ...
                    children: [
                      _Kpi(
                          ...
                          value: formatCompactInt(
                              store.auditSummary!.totalAuditEvents)),
                      _Kpi(
                          label: '24h',
                          value: formatCompactInt(
                              store.auditSummary!.auditEvents24h)),
                      _Kpi(
                          label: 'Failed 24h',
                          value: formatCompactInt(
                              store.auditSummary!.failedAdminActions24h)),
                      _Kpi(
                          label: 'Latest',
                          value: formatDateTimeShort(
                              store.auditSummary!.latestAuditEventAt)),
                    ],
                  ),
                ),
              ),
                ...
                children: [
                  SizedBox(
                    ...
                    child: TextField(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'Admin user id'),
                      onChanged: (v) => store.setAuditLogQuery(
                          store.auditLogQuery.copyWith(
                              adminUserId: v.trim(),
                              clearAdminUserId: v.trim().isEmpty)),
                    ),
                  ),
                  SizedBox(
                    width: 260,
                    child: TextField(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'Target user id'),
                      onChanged: (v) => store.setAuditLogQuery(
                          store.auditLogQuery.copyWith(
                              targetUserId: v.trim(),
                              clearTargetUserId: v.trim().isEmpty)),
                    ),
                  ),
                  _SelectFilterChip(
                    label: 'Action type',
                    ...
                    onChanged: (v) => store.setAuditLogQuery(store.auditLogQuery
                        .copyWith(actionType: v, clearActionType: v == null)),
                  ),
                  _SelectFilterChip(
                    label: 'Result',
                    ...
                    onChanged: (v) => store.setAuditLogQuery(store.auditLogQuery
                        .copyWith(result: v, clearResult: v == null)),
                  ),
                  _DateFilterChip(
                    label: 'Date',
                    range: store.auditLogQuery.createdRange,
                    onChanged: (r) => store.setAuditLogQuery(store.auditLogQuery
                        .copyWith(
                            createdRange: r, clearCreatedRange: r == null)),
                  ),
                  TextButton(
                    onPressed: () async {
                      _adminCtrl.text = '';
  ...
  Widget build(BuildContext context) {
    ...
    return ExpansionTile(
      ...
      leading: Container(
        ...
        child: Icon(isFail ? Icons.error_outline : Icons.fact_check_outlined,
            color: isFail ? cs.onErrorContainer : cs.onSurfaceVariant,
            size: 18),
      ),
      ...
      children: [
        Wrap(
          ...
          children: [
            ...
            _MetaChip(
                label: 'UA',
                value: entry.userAgent == null
                    ? '—'
                    : _truncate(entry.userAgent!, 40)),
          ],
        ),
        const SizedBox(height: AppSpacing.md),
        ...
        if ((entry.previousValue != null) || (entry.newValue != null)) ...[
          ...
          Row(
            children: [
              Expanded(
                  child: _JsonPanel(
                      title: 'Previous (redacted)',
                      value: AdminAuditRedactor.jsonStringOrNull(
                          entry.previousValue))),
              ...
              Expanded(
                  child: _JsonPanel(
                      title: 'New (redacted)',
                      value:
                          AdminAuditRedactor.jsonStringOrNull(entry.newValue))),
            ],
          ),
        ...
        children: [
          Text(title,
              style: Theme.of(context).textTheme.labelLarge?.copyWith(
                  fontWeight: FontWeight.w800, color: cs.onSurfaceVariant)),
...
class _DateFilterChip extends StatelessWidget {
  const _DateFilterChip(
      {required this.label, required this.range, required this.onChanged});
  final String label;
  ...
  Widget build(BuildContext context) {
    ...
    final text = range == null
        ? 'Any'
        : '${AdminFormatters.date(range!.start)} → ${AdminFormatters.date(range!.end)}';
