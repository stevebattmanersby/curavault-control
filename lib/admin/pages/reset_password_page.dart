...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _recoverFromUrl() async {
    ...
    try {
      ...
      //
      // supabase_flutter exposes a helper for this.
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _info = 'Password updated. You can now sign in.';
      });

      // Send the user back to login. If they already have an authorized admin
      // session, router redirects will take them to the shell.
      context.go(AppRoutes.login);
    } catch (e) {
      debugPrint('[ResetPasswordPage] updateUser failed: $e');
      if (!mounted) return;
