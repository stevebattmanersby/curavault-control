...
class SupportSessionDetailPage extends StatefulWidget {
  ...
  @override
  State<SupportSessionDetailPage> createState() =>
      _SupportSessionDetailPageState();
}
...
class _SupportSessionDetailPageState extends State<SupportSessionDetailPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await context
          .read<AdminStore>()
          .getSupportSessionDetail(widget.supportSessionId);
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return AdminPageScaffold(
      ...
      subtitle:
          'Diagnostics and account metadata only. Health content is never displayed.',
      actions: [
        TextButton.icon(
          onPressed: () => context.go(
              '/support/diagnostics${detail == null ? '' : '?userId=${Uri.encodeComponent(detail.userId)}'}'),
        ...
        OutlinedButton.icon(
          onPressed: detail == null
              ? null
              : () => _openActionSheet(SupportAction.closeSupportSession),
              ...
              ? AdminCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                          'Try returning to the queue and selecting a session again.',
                          style: Theme.of(context)
                              .textTheme
                              .bodyMedium
                      ...
                      OutlinedButton.icon(
                        ...
                        label:
                            Text('Back', style: TextStyle(color: cs.onSurface)),
                      ),
                    ],
                  ),
                )
                  ...
                  children: [
                    Wrap(
                      ...
                      children: [
                        _MiniStat(
                            label: 'Session ID',
                            value: detail.supportSessionId),
                        _MiniStat(label: 'Status', value: detail.status.label),
                        _MiniStat(
                            label: 'Consent',
                            value: detail.consentWindowStatus),
                        _MiniStat(
                            label: 'Access expires',
                            value: formatDateTimeShort(detail.accessExpiresAt)),
                        _MiniStat(
                            label: 'Assigned',
                            value: detail.assignedAdmin ?? '—'),
                        _MiniStat(
                            label: 'Last updated',
                    ...
                    _TwoCol(
                      left:
                          _SessionUserCard(detail: detail, canEmail: canEmail),
                    ...
                    _TwoCol(
                      ...
                      right:
                          _ActionsCard(role: role, onAction: _openActionSheet),
                    ),
                    const SizedBox(height: AppSpacing.md),
                    AdminCard(
                      header: Text('Recent technical events',
                          style: Theme.of(context)
                              .textTheme
                              .titleMedium
                              ?.copyWith(fontWeight: FontWeight.w800)),
                    ...
                    const SizedBox(height: AppSpacing.md),
                    AdminCard(
                      header: Text('Diagnostics checker',
                          style: Theme.of(context)
                              .textTheme
                              .titleMedium
                              ?.copyWith(fontWeight: FontWeight.w800)),
                    ...
                    AdminCard(
                      ...
                      child: Text(
                        detail.adminNotes?.trim().isNotEmpty == true
                            ? 'Present (redacted)'
                            : '—',
                        style: Theme.of(context)
                            .textTheme
    ...
    return AdminCard(
      ...
      child: Column(
        children: [
          ...
          if (canEmail)
            _KV(label: 'Email', value: detail.email ?? '—')
          else
            _KV(label: 'Email', value: 'Hidden by role'),
          ...
          _KV(
              label: 'Last login',
              value: formatDateTimeShort(detail.lastLoginAt)),
          _KV(
              label: 'Last sync',
              value: formatDateTimeShort(detail.lastSyncAt)),
          _KV(
              label: 'Failed sync count',
              value: detail.failedSyncCount.toString()),
          _KV(
              label: 'Failed upload count',
              value: detail.failedUploadCount.toString()),
    ...
    return AdminCard(
      ...
      child: Column(
        children: [
          _KV(
              label: 'Storage used',
              value: formatBytes(detail.storageUsedBytes)),
          _KV(
              label: 'Storage limit',
              value: formatBytes(detail.storageLimitBytes)),
          _KV(
              label: 'AI tokens used (mo)',
              value: formatCompactInt(detail.aiTokensUsed)),
...
class _OpenErrorsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return AdminCard(
      header: Text('Open errors',
          style: Theme.of(context)
              .textTheme
              .titleMedium
              ?.copyWith(fontWeight: FontWeight.w800)),
      child: openErrors.isEmpty
          ? Text('No open errors flagged.',
              style: Theme.of(context)
                  .textTheme
                  .bodyMedium
                  ?.copyWith(color: cs.onSurfaceVariant))
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(
          horizontal: AppSpacing.md, vertical: AppSpacing.sm),
      decoration: BoxDecoration(
          color: cs.errorContainer, borderRadius: BorderRadius.circular(999)),
      child: Text(code,
...
class _DiagnosticsPanel extends StatelessWidget {
  const _DiagnosticsPanel(
      {required this.report, required this.isLoading, required this.onRun});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            FilledButton.icon(
              ...
              label: Text(isLoading ? 'Running…' : 'Run checks',
        ...
        const SizedBox(height: AppSpacing.md),
        if (rep == null)
          Text('Run checks to generate a privacy-safe diagnostic report.',
              style: Theme.of(context)
                  .textTheme
                  .bodyMedium
                  ?.copyWith(color: cs.onSurfaceVariant))
...
class SupportActionConfirmSheet extends StatefulWidget {
  ...
  @override
  State<SupportActionConfirmSheet> createState() =>
      _SupportActionConfirmSheetState();
}
  ...
  Widget build(BuildContext context) {
    ...
    String paramLabel() => switch (widget.action) {
          ...
          SupportAction.temporarilyIncreaseStorageLimit =>
            'New storage limit (bytes)',
          SupportAction.temporarilyIncreaseAiLimit =>
            'New AI token limit (month)',
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: AppSpacing.sm),
              Text('Support session: ${widget.supportSessionId}',
                  style: Theme.of(context)
                      .textTheme
                      .labelMedium
                      ?.copyWith(color: cs.onSurfaceVariant)),
              Text('Target user: ${widget.userId}',
                  style: Theme.of(context)
                      .textTheme
                      .labelMedium
                      ?.copyWith(color: cs.onSurfaceVariant)),
              ...
              CheckboxListTile(
                ...
                title: Text('I confirm this is appropriate and auditable',
                ...
                children: [
                  ...
                  Expanded(
                    child: FilledButton(
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                    content: Text(
                                        'Action completed: ${widget.action.label}')));
                                context.pop();
                              } catch (e) {
                      ...
                      child: Text(_submitting ? 'Working…' : 'Confirm action',
                          style: TextStyle(color: cs.onPrimary)),
                    ),
                  ),
                ],
